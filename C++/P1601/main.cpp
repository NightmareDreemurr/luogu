...
string high_def_add(string numa, string numb) {
    ...
    // for(int i = 0; i < min(numa.size(), numb.size()); i++) {
    //     new_num_vector.push_back(numb[i] + numa[i] - '0' - '0');
    // }
    // for(int i = min(numa.size(), numb.size()); i < max(numa.size(), numb.size()); i++) {
    //     if(numa.size() > numb.size()) {
    //         new_num_vector.push_back(numa[i] - '0');
    //     }
    // }
